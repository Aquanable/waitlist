...
Every fleet has specialty roles that need to be performed to allow our fleets to finish sites quickly and effectively. The roles range from managing your fleet members drones to acting as bait for an entire site. Irregardless of role, each is vital to our composition, and depending on your chosen ship you may be asked to fill the role so the fleet can continue to get paid. Remember - when you join fleet you are automatically signing on to roles specific to your ship! If an FC asks for a role to be filled you should x-up for it immediately by typing "x DDD" etc. for respective roles. It is much appreciated!
...
As DDD you will be responsible for killing frigates in all sites every wave. While challenging, DDD can be by far the most rewarding role. We recommend a high level of Minmatar battleship, and to be fitted with 3 webifiers. While in this role you should recieve tracking links from (at minimum) your oneiros logi: this will help to improve tracking speed and optimal range for your guns. If you are not recieving links, speak up!
...
There is a priority order to frigates, but remember to always shoot the frigates closest to optimal, and with the lowest transversal. Uou will kill them much faster this way so keep checking your overview, and memorise your optimal ranges with each ammo type.
...
- Niarjas are your **TOP** priority in this site, this is because they Jam & Neut, they also respawn. When you land on the beacon sit still, and using Null, quickly kill both the Niarjas (with all the links you can easily kill even the one startinhg at 100 km, as it will butn towards you). Next spread your webs for nearby Schmaeels but dont bother with the tag X's. Switch to Void or Antimatter (if you have the range) and try to kill off as many Schmaeels as you can before the aggro switchs. When aggro switches the remaining Schmaeels will burn toward the tower so align and MJD to the left of it. When you land kill respawned Niarjas & between Niarja respawns kill off untagged Schmaeels.
...
Keep reactivating or "cycling" your guns. When you enter a site, pilots will gradually add more and more drones to your pool, as some pilots are slower than others. Check back on occasion to see if you have any idle drones orbiting you, and if so turn your guns/web off and re-apply them to get those drones shooting.
...
Try to take into account the distance to a target, and whether the time taken for drones to travel a long distance would be worthwhile. The last ship in a wave 60km away would not really benefit from the drones as it would most likely be killed before they arrive. This leaves the drones having to travel back to you as the next wave spawns and their overall application time is reduced even further than if you had simply let them sit idle next to you.

## MTAC/CIVI

Your role as MTAC is to stop the tower from repairing during the TCRC site. To do this the pilot ia required to transfer an item called the "MTAC" from its factory to the Shield Array Tower. This process must be repeated until the tower is destroyed.
...
You will need to have your Tractor Beam equipped. You want to be one of the first people on grid when the FC calls the gate green. Align and MJD to the MTAC factory as soon as you land. With the Bastion changes it is alot more important to get the first drop done as soon as possible as pilots will be jumping from the beacon once the Deltoles are dead (if not sooner).
...
When you land at the MTAC factory jettison an item from your cargo, lock, open, and tractor your can. Put the can in the second row of your locks so you dont accidentally shoot it yourself. Open the MTAC factory and drop the MTAC into your can, then burn over to the Shield Array Tower, once within 2500m drop the MTAC into the Shield Array Tower (be carefull not to bounce too far away from the Tower on approach). You can call out "dropped" in comms to let the fleet/FC know. At this point lock the tower and shoot it to secure your payout. Finally, burn back to the MTAC factory and repeat the process until the tower is destroyed. **Never stop dropping**.
